...
import type { MarketListQuery } from '@vegaprotocol/market-list';
import {
...
describe('market list', { tags: '@smoke' }, () => {
  describe('simple url', () => {
    beforeEach(() => {
      cy.mockGQL((req) => {
        aliasQuery(req, 'MarketList', generateSimpleMarkets());
  ...
  describe('url params should select filters', () => {
    ...
    it('last asset (if exists)', () => {
      ...
      cy.wait('@Markets').then((filters) => {
        const data: MarketListQuery | undefined = filters?.response?.body?.data;
  ...
  describe('long list of results should be handled properly', () => {
    it('handles 1000 markets', () => {
      ...
      cy.mockGQL((req) => {
        aliasQuery(req, 'MarketList', generateLongListMarkets(1000));
        aliasQuery(req, 'MarketsData', generateMarketsData());
        aliasQuery(req, 'MarketsCandles', generateMarketsCandles());
      });
