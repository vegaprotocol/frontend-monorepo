  ...
  describe('Verify parties page content', function () {
        ...
        cy.contains(assetData[asset].name, txTimeout)
          ...
          .within(() => {
            cy.get_asset_decimals(asset).then(assetDecimals => {
              cy.contains_exactly(assetData[asset].amount + '.' + assetDecimals).should('be.visible');
            })
          })
      })
    });
    ...
    it('should be able to copy the party address id', function () {
      ...
      cy.contains('Address')
        ...
        .last()
        .click()
      cy.get('@clipboard').get_copied_text_from_clipboard()
    ...
    it('should be able to copy an asset id', function () {
      ...
      cy.get('@clipboard').get_copied_text_from_clipboard().should('equal', assetData.fDAI.id);
    });
    ...
    it('should be able to see JSON of each asset containing correct balance and decimals', function () {
      ...
      assetsInTest.forEach(asset => {
        cy.get(partiesJsonSection)
          ...
          .then(accounts => {
            cy.get_asset_decimals(asset).then(assetDecimals => {
              assert.equal(accounts[asset].balance, assetData[asset].amount + assetDecimals);
            })
          })
      })
    });
    ...
    Cypress.Commands.add('convert_string_json_to_js_object',
    { prevSubject: true }, (jsonBlobString) => {
      return JSON.parse(jsonBlobString);
    });

    Cypress.Commands.add('get_party_accounts_data',
    { prevSubject: true }, (jsObject) => {
      const accounts = jsObject.party.accounts.reduce(function (account, entry) {
