...
export default class HomePage extends BasePage {
  ...
  validateStringIsDisplayedAtTopOfTable(value: string) {
    // Ignore header row
    cy.get('table tr').eq(1).within(() => cy.contains(value).should('be.visible'))
  }
  ...
  getOldestTradableInstrument(openMarkets:[]) {
    ...
    openMarkets.forEach((market:[]) => {
      let epochTimestamp = Date.parse(market.marketTimestamps.open);
  ...
  validateTableCodesExistOnServer(openMarketCodes:[]) {
    cy.get('table tr').each(($element, index) => {
      if (index > 0) { // skip header row
        ...
        assert.include(openMarketCodes, openMarketCodeText, `Checking ${openMarketCodeText} is shown within server open markets response`);
      }
    })
  }
