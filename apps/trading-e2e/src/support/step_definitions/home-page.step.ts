...
Then('the server contains at least {int} open markets', function(expectNumber) {
  expect(this.openMarketData.length).to.be.at.least(expectNumber); 
});

Then('each market shown in overlay table exists as open market on server', function() {
  homePage.getOpenMarketCodes(this.openMarketData)
    .then((openMarketCodes) => {
      homePage.validateTableCodesExistOnServer(openMarketCodes);
    });
});

Then('the oldest market trading in continous mode shown at top of overlay table', function() {
  homePage.getOldestTradableInstrument(this.openMarketData)
    .then((oldestTradableInstrument) => {
      homePage.validateStringIsDisplayedAtTopOfTable(oldestTradableInstrument.instrument.code);
    });
});

Then('the oldest current trading market is loaded on the trading tab', function () {
  homePage.getOldestTradableInstrument(this.openMarketData)
    .then((oldestTradableInstrument) => {
      cy.get('h1')
        .should('contain', oldestTradableInstrument.instrument.name)
    });
})

Then('the newest current trading market is loaded on the trading tab', function () {
  homePage.getMostRecentTradableInstrument(this.openMarketData)
    .then((mostRecentMarketInstrument) => {
      cy.get('h1')
        .should('contain', mostRecentMarketInstrument.instrument.name)
    });
})
