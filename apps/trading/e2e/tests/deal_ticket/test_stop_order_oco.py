...
def test_submit_stop_order_market_oco_rejected(
    ...
    create_and_faucet_wallet(vega=vega, wallet=stop_order_market_oco_rejected)
    page.goto(f"/#/markets/{shared_continuous_market}")
    change_keys(page, vega, stop_order_market_oco_rejected.name)
    page.get_by_test_id(stop_orders_tab).click()
    page.get_by_test_id(stop_order_btn).click()
    page.get_by_test_id(stop_market_order_btn).is_visible()
    page.get_by_test_id(stop_market_order_btn).click()
    page.get_by_test_id(trigger_price).fill("1")
    ...
    # 7002-SORD-086
    page.get_by_test_id(trigger_price_oco).fill("200")
...
def test_submit_stop_oco_market_order_triggered(
    ...
    create_and_faucet_wallet(vega=vega, wallet=stop_oco_market_order_triggered)
    create_position(
        vega, shared_continuous_market, stop_oco_market_order_triggered.name
    )

    page.goto(f"/#/markets/{shared_continuous_market}")
    change_keys(page, vega, stop_oco_market_order_triggered.name)
    page.get_by_test_id(stop_orders_tab).click()
...
def test_submit_stop_oco_market_order_pending(
    ...
    create_and_faucet_wallet(vega=vega, wallet=stop_oco_market_order_pending)
    create_position(vega, shared_continuous_market, stop_oco_market_order_pending.name)

    page.goto(f"/#/markets/{shared_continuous_market}")
    change_keys(page, vega, stop_oco_market_order_pending.name)
    page.get_by_test_id(stop_orders_tab).click()
    page.get_by_test_id(stop_order_btn).click()
    page.get_by_test_id(stop_market_order_btn).is_visible()
    page.get_by_test_id(stop_market_order_btn).click()
...
def test_submit_stop_oco_limit_order_pending(
    ...
    create_and_faucet_wallet(vega=vega, wallet=stop_oco_limit_order_pending)
    create_position(vega, shared_continuous_market, stop_oco_limit_order_pending.name)

    page.goto(f"/#/markets/{shared_continuous_market}")
    change_keys(page, vega, stop_oco_limit_order_pending.name)
    page.get_by_test_id(stop_orders_tab).click()
    page.get_by_test_id(stop_order_btn).click()
...
def test_submit_stop_oco_limit_order_cancel(
    ...
    create_and_faucet_wallet(vega=vega, wallet=stop_oco_limit_order_cancel)
    create_position(vega, shared_continuous_market, stop_oco_limit_order_cancel.name)

    page.goto(f"/#/markets/{shared_continuous_market}")
    change_keys(page, vega, stop_oco_limit_order_cancel.name)
    page.get_by_test_id(stop_order_btn).click()
    page.get_by_test_id(stop_limit_order_btn).is_visible()
    page.get_by_test_id(stop_limit_order_btn).click()
    page.get_by_test_id(order_side_sell).click()
