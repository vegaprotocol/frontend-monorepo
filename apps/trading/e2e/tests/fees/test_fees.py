...
market_id_tier_1_volume = "default_id"
market_id_tier_2_volume = "default_id"
market_id_tier_1_referral = "default_id"
market_id_tier_2_referral = "default_id"
market_id_combo = "default_id"
...
@pytest.fixture
def vega_instance(tier, discount_program, 
                  vega_volume_discount_tier_1, vega_volume_discount_tier_2, 
                  vega_referral_discount_tier_1, vega_referral_discount_tier_2, vega_referral_and_volume_discount):
    if discount_program == "volume":
        ...
        return vega_referral_discount_tier_1 if tier == 1 else vega_referral_discount_tier_2
...
def set_market_volume_discount(vega, tier, discount_program):
    global market_id_tier_1_volume
    global market_id_tier_2_volume
    global market_id_tier_1_referral
    global market_id_tier_2_referral
    global market_id_combo

    if discount_program == "volume":
        market_id = market_id_tier_1_volume if tier == 1 else market_id_tier_2_volume
    elif discount_program == "referral":
        market_id = market_id_tier_1_referral if tier == 1 else market_id_tier_2_referral
    elif discount_program == "combo":
        market_id = market_id_combo

    print(f"Checking if market exists: {market_id}")
    if not market_exists(vega, market_id):
        print(f"Market doesn't exist for {discount_program} tier {tier}. Setting up new market.")
        if discount_program == "volume":
        ...
        if tier == 1 and discount_program == "volume":
            market_id_tier_1_volume = market_id
        elif tier == 2 and discount_program == "volume":
            market_id_tier_2_volume = market_id
        elif tier == 1 and discount_program == "referral":
            market_id_tier_1_referral = market_id
        elif tier == 2 and discount_program == "referral":
            market_id_tier_2_referral = market_id
        elif discount_program == "combo":
            market_id_combo = market_id

    print(f"Using market ID: {market_id}")
    return market_id

@pytest.mark.parametrize("tier, discount_program, expected_text", [
...
@pytest.mark.usefixtures("risk_accepted", "auth", "page")
def test_fees_page_discount_program_my_trading_fees(tier, expected_text, discount_program, vega_instance, page: Page):
        set_market_volume_discount(vega_instance, tier, discount_program)
        page.goto("/#/fees")
        expect(page.get_by_test_id(ADJUSTED_FEES)).to_have_text(expected_text)
        expect(page.get_by_test_id(TOTAL_FEE_BEFORE_DISCOUNT)).to_have_text("Total fee before discount10.05%-10.05%")
...
@pytest.mark.parametrize("tier, discount_program, volume_discount, total_discount, referral_discount", [
...
@pytest.mark.usefixtures("risk_accepted", "auth", "page")
def test_fees_page_discount_program_total_discount(tier, discount_program, volume_discount, referral_discount, total_discount, vega_instance, page: Page):
        set_market_volume_discount(vega_instance, tier, discount_program)
        page.goto("/#/fees")
        ...
        expect(page.get_by_test_id(TOOLTIP_CONTENT).nth(0)).to_have_text("The total discount is calculated according to the following formula: 1 - (1 - dvolume) ⋇ (1 - dreferral)")

@pytest.mark.parametrize("tier, discount_program, past_epochs_volume, required_for_next_tier", [
    (1, "volume", "103", "97"),
    (2, "volume", "206", "")
])
@pytest.mark.usefixtures("page", "risk_accepted", "auth")
def test_fees_page_volume_discount_program_my_current_volume(tier, discount_program, past_epochs_volume, required_for_next_tier, vega_instance, page: Page):
    set_market_volume_discount(vega_instance, tier, discount_program)
    page.goto("/#/fees")
    expect(page.get_by_test_id(PAST_EPOCHS_VOLUME)).to_have_text(past_epochs_volume)
    
    if tier == 1:
        expect(page.get_by_test_id(REQUIRED_FOR_NEXT_TIER)).to_have_text(required_for_next_tier)
    else:
        expect(page.get_by_test_id(REQUIRED_FOR_NEXT_TIER)).not_to_be_visible()

@pytest.mark.parametrize("tier, discount_program, notional_taker_volume, epochs_in_set", [
    (1, "referral", "103", "1"),
    (2, "referral", "207", "1")
])
@pytest.mark.usefixtures( "page", "risk_accepted", "auth")
def test_fees_page_referral_discount_program_referral_benefits(tier, vega_instance, discount_program, notional_taker_volume, epochs_in_set, page: Page):
    set_market_volume_discount(vega_instance, tier, discount_program)
    page.goto("/#/fees")
    expect(page.get_by_test_id(RUNNING_NOTIONAL_TAKER_VOLUME)).to_have_text(notional_taker_volume)
    expect(page.get_by_test_id(EPOCHS_IN_REFERRAL_SET)).to_have_text(epochs_in_set)

@pytest.mark.parametrize("tier, discount_program, my_volume_test_id, my_volume_value, your_tier", [
...
@pytest.mark.usefixtures("page", "risk_accepted", "auth")
def test_fees_page_discount_program_discount(tier, discount_program, my_volume_test_id, my_volume_value, your_tier, vega_instance, page: Page):
    set_market_volume_discount(vega_instance, tier, discount_program)
    page.goto("/#/fees")
...
@pytest.mark.parametrize("tier, discount_program, discount, discount_value, total_fee", [
...
@pytest.mark.usefixtures("page", "risk_accepted", "auth")
def test_deal_ticket_discount_program(tier, discount_program, discount, discount_value, total_fee, vega_instance, page: Page):
    market = set_market_volume_discount(vega_instance, tier, discount_program)
...
@pytest.mark.parametrize("tier, discount_program, fee, fee_discount, price_1, size", [
    (1, "volume", "9.36158 tDAI", "1.04017 tDAI", "103.50 tDAI", "+1"),
    (2, "volume", "8.3214 tDAI", "2.08035 tDAI", "103.50 tDAI", "+1"),
    (1, "referral", "8.42543 tDAI ", "1.04017 tDAI", "103.50 tDAI", "+1"),
    (2, "referral", "13.31424 tDAI", "4.1607 tDAI", "207.00 tDAI", "+2"),
    (2, "combo", "10.6514 tDAI ", "7.48926 tDAI", "207.00 tDAI", "+2")
])
@pytest.mark.usefixtures("page", "risk_accepted", "auth")
def test_fills_taker_discount_program(tier, discount_program, fee, fee_discount, price_1, size, vega_instance, page: Page):
    market = set_market_volume_discount(vega_instance, tier, discount_program)
    page.goto(f"/#/markets/{market}")
...
@pytest.mark.parametrize("tier, discount_program, fee, fee_discount, size, price_1", [
...
@pytest.mark.usefixtures("page", "risk_accepted", "auth")
def test_fills_maker_discount_program(tier, discount_program, vega_instance, fee, fee_discount, size, price_1, page: Page):
    market = set_market_volume_discount(vega_instance, tier, discount_program)
    page.goto(f"/#/markets/{market}")
...
@pytest.mark.parametrize("tier, discount_program, fee", [
    (1, "volume", "9.315"),
    (2, "volume", "8.28"),
    (1, "referral", "8.3835"),
    (2, "referral", "13.248"),
...
@pytest.mark.usefixtures("page", "risk_accepted", "auth")
def test_fills_maker_fee_tooltip_discount_program(tier, discount_program, fee, vega_instance, page: Page):
    market = set_market_volume_discount(vega_instance, tier, discount_program)
    page.goto(f"/#/markets/{market}")
    change_keys(page, vega_instance, MM_WALLET.name)
    page.get_by_test_id(FILLS).click()
    ...
    expect(page.get_by_test_id(FEE_BREAKDOWN_TOOLTIP)).to_have_text(f"If the market was activeThe maker will receive the maker fee.If the market is active the maker will pay zero infrastructure and liquidity fees.Infrastructure fee0.00 tDAILiquidity fee0.00 tDAIMaker fee-{fee} tDAITotal fees-{fee} tDAI")

@pytest.mark.parametrize("tier, discount_program, maker_fee, total_fee, infra_fee", [
...
@pytest.mark.usefixtures("page", "risk_accepted", "auth")
def test_fills_taker_fee_tooltip_discount_program(tier, discount_program, vega_instance, maker_fee, total_fee, infra_fee, page: Page):
    market = set_market_volume_discount(vega_instance, tier, discount_program)
    page.goto(f"/#/markets/{market}")
    page.get_by_test_id(FILLS).click()
    row = page.get_by_test_id(TAB_FILLS).locator(ROW_LOCATOR).first
    row.locator(COL_FEE).hover()
    expect(page.get_by_test_id(FEE_BREAKDOWN_TOOLTIP)).to_have_text(f"If the market was activeFees to be paid by the taker.Infrastructure fee{infra_fee} tDAILiquidity fee0.00 tDAIMaker fee{maker_fee} tDAITotal fees{total_fee} tDAI")