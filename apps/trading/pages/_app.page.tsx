...
function VegaTradingApp(props: AppProps) {
  ...
  useEffect(() => {
    const run = async () => {
      const walletsStore = createSessionStorage<WalletStore>('wallets');
      const publicKeyIndexStore =
        createSessionStorage<KeyInfoStore>('public-key-index');
      const keySortIndex = createSessionStorage('key-sort-index');
      const walletCollection = new WalletCollection({
        walletsStore,
        publicKeyIndexStore,
        keySortIndex,
      });
      console.log('All instasiated');
      const recoveryPhrase = await walletCollection.generateRecoveryPhrase();
      console.log('Phrase:', recoveryPhrase);
      await walletCollection.import({
        recoveryPhrase: recoveryPhrase,
        name: 'test-wallet',
      });
      console.log('Wallet generated');
      await walletCollection.generateKey({
        wallet: 'test-wallet',
      });
      await walletCollection.generateKey({
        wallet: 'test-wallet',
      ...
      const keys = await walletCollection.listKeys({
        wallet: 'test-wallet',
      });
