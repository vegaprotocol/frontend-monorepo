...
export class NetworkCollection {
  ...
  async get(networkId, chainId) {
    ...
    const candidate =
      (await this.getByNetworkId(networkId)) ??
      (await this.getByChainId(chainId));

    if (candidate == null)
      throw new Error(
        `No network found for networkId ${networkId} or chainId ${chainId}`
      );
...
export class Network {
  ...
  async rpc() {
    ...
    this.preferredNode = NodeRPC.findHealthyNode(
      this.rest.map((u) => new URL(u))
    )
      .then(
        (node) => {
