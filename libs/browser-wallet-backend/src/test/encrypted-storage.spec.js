...
describe('encrypted-storage', () => {
  it('Invalid passphrase', async () => {
    const store = new Map();
    const encryptedStorage = new EncryptedStorage(store, {
      memory: 10,
      iterations: 1,
    });
    ...
    await expect(encryptedStorage.unlock('invalid')).rejects.toThrow(
  ...
  it('Storage is not open', async () => {
    ...
    await expect(encryptedStorage.get('foo')).rejects.toThrow(
      'Storage is locked'
    );
  });
  ...
  it('Create storage twice', async () => {
    ...
    await expect(encryptedStorage.create('passphrase')).rejects.toThrow(
