...
type StatusComponentProps = Pick<ComponentProps<typeof Dialog>, 'intent' | 'title' | 'children'>;

const getStatusDialogProps = (t: Translate, status: ConfigStatus): StatusComponentProps => {
  switch (status) {
    case 'error-loading-config': return {
      title: t('Error'),
      intent: Intent.Danger,
      children: <>{t('There was an error ')}</>,
    };
    case 'error-loading-node': return {
      title: t('Error'),
      intent: Intent.Danger,
      children: <>{t('There was an error ')}</>,
    };
    case 'loading-config': return {
      title: t('Error'),
      intent: Intent.Progress,
      children: <>{t('Loading configuration...')}</>,
    };
    case 'loading-node': return {
      title: t('Error'),
      intent: Intent.Progress,
...
export function NetworkLoader<T> ({ children, createClient }: NetworkLoaderProps<T>) {
