...
const update = (data: (TradesItem | null)[], delta: TradeFieldsFragment[]) => {
...
const getData = (responseData: TradesQuery): TradesItem[] | null =>
  responseData.market ? responseData.market.tradesConnection.edges : null;

const getDelta = (
  subscriptionData: TradesEventSubscription
): TradeFieldsFragment[] => subscriptionData?.trades || [];
