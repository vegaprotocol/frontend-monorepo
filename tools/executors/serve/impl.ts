...
import devServerExecutor, { WebDevServerOptions } from '@nrwl/web/src/executors/dev-server/dev-server.impl';
...
const getEnvFile = (env: string, envFiles: string[]) => {
  const envMap = envFiles.reduce((acc, filename) => ({
    ...acc,
...
export default async function* serve(
  ...
  const envFile = getEnvFile(env, files.filter(f => f.startsWith('.env')));

  if (envFile) {
    dotenv.config({ path: path.join(workspacePath, envFile) })
  }

  return devServerExecutor(dsOptions, context);
}
