...
process.env['NX_GIT_COMMIT_HASH'] = (0, child_process_1.execSync)('git rev-parse HEAD')
    .toString()
    .replace(/[\r\n]/gm, '');
process.env['NX_GIT_ORIGIN_URL'] = (0, child_process_1.execSync)('git remote get-url origin')
    .toString()
...
var logEnvData = function (envMap, envFiles, env, defaultEnvFile, loggerScope) {
    if (env && !envMap[env]) {
        ...
        log.info(loggerScope, envFiles.length > 0
    ...
    if (!envMap[env]) {
        log.info(loggerScope, defaultEnvFile
        ...
        log.info(loggerScope, "Using \"".concat(envMap[env], "\" as the default project environment."));
    }
};
    ...
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    files = _a.sent();
                    envFile = getEnvFile(env, files.filter(function (f) { return f.startsWith('.env'); }), loggerScope);
                    if (env && envFile) {
