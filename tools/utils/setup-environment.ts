...
try {
  process.env['NX_GIT_COMMIT_HASH'] = process.env['COMMIT_REF'] ?? execSync('git rev-parse HEAD')
    .toString()
    .replace(/[\r\n]/gm, '');
  process.env['NX_GIT_BRANCH'] = process.env['BRANCH'] ?? execSync('git rev-parse --abbrev-ref HEAD')
    .toString()
    .replace(/[\r\n]/gm, '');
  process.env['NX_GIT_ORIGIN_URL'] = process.env['REPOSITORY_URL'] ?? execSync('git remote get-url origin')
    .toString()
